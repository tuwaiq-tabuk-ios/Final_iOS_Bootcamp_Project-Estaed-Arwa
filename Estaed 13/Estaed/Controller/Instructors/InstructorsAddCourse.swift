...
class InstructorsAddCourse : UIViewController {
  ...
  // formatter
    ...
    if error != nil {
        Constant.getMsg(title: NSLocalizedString("Error!", comment: ""), Msg: NSLocalizedString("Error In Validating Data", comment: ""), context: self)
    } else {
      //سجل بقاعده البيانات
      ...
      self.ref.child("courses").child(instructorsId).child(course.courseId).setValue(course.toDic()) { Error, DataRef in
        ...
        if Error == nil
        {
            Constant.getMsg(title: NSLocalizedString("Nice Work", comment: ""), Msg:
                            NSLocalizedString("Course Added Successfully", comment: ""), context: self){  result, error in
                if !error
                {
                    self.dismiss(animated: true, completion: nil)
