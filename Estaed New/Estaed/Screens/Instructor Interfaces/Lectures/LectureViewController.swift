...
class LectureViewController: UIViewController {
  ...
  @IBAction func DateChanged(_ sender: UIDatePicker)
 {
  ...
  @IBAction func addBtn(_ sender: Any) {
    ...
    if error != nil {
      K.getMsg(title:"Error!".localized, Msg: "Error In Validating Data".localized, context: self)
    
    } else {
      let LectId = UUID.init().uuidString
      
        let LectureModelList = LectureModel(LectureName: self.LectureName.text!, lectureURL: self.lectureLink.text! ,lectureTime: self.time, lectureDate: self.date,courseId: self.courseId )
        ref = Database.database().reference()
      self.ref.child(K.RealtimeDatabase.lectures).child(LectureModelList.courseId).child(LectId).setValue(LectureModelList.toDic()) {Error, DataRef in
      
        if Error == nil
        {
          K.getMsg(title:"Nice Work".localized, Msg: "lecture Added Successfully".localized, context: self){  result, error in
            if !error
            {
