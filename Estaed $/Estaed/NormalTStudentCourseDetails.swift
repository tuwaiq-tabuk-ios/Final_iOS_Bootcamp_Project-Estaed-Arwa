...
class NormalTStudentCourseDetails : UIViewController {
  
  
   @IBOutlet weak var TeatcherNameTStudent: UILabel!
  ...
  func getDetailsOfCourses(){
    ...
    ref.child("courses").child("Tahsile").child("Normal").observe(.value) { (snapshot) in
   ...
   let TeatcherNameStudent = snapshotVaue?["addteachername"] as? String
      ...
      self.TeatcherNameTStudent.text = TeatcherNameStudent
