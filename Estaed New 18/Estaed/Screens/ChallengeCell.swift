  ...
  var difficultyLevelLabel : UILabel = {
   let label = UILabel(frame: CGRect(x: 230, y: 60, width: 200, height: 25))
    ...
    var BtnChallengDetails: UIButton = {
      ...
      let btn = UIButton(frame: CGRect(x: 230, y: 110, width: 150, height: 25))
     
//      btn.addTarget(self, action:  #selector(BtnChallengDetails(_:)), for: .touchUpInside)
      btn.setTitle("Challenge Details", for: .normal)
  ...
  func initCell(model : ChallengAttachmentModel)
  {
    ...
    self.BtnChallengDetails.addTarget(self, action:  #selector(BtnChallengDetails(_:)), for: .touchUpInside)
